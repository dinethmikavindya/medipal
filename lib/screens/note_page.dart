...
class _NotePageState extends State<NotePage> with SingleTickerProviderStateMixin {
  ...
  // ✅ FIRESTORE: These two lines connect to Firebase
  final _firestore = FirebaseFirestore.instance;
  final _auth = FirebaseAuth.instance;

  // ✅ FIRESTORE: This builds the path: users → your user id → notes
  ...
  // ✅ FIRESTORE: note now carries an 'id' field so we know which doc to update
  void _openNoteEditor({bool isEdit = false, Map<String, dynamic>? note}) {
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    try {
                      // ✅ FIRESTORE: If editing, update the existing doc. If new, add a new doc.
                      if (isEdit && note?['id'] != null) {
                        await _notesRef.doc(note!['id']).update({
                          "title": titleController.text.trim(),
                          "content": contentController.text.trim(),
                          "date": FieldValue.serverTimestamp(), // ✅ Firebase sets the time
                        });
                      } else {
                        await _notesRef.add({
                          "title": titleController.text.trim(),
  ...
  // ✅ FIRESTORE: Now takes a noteId (text) instead of an index number
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              // ✅ FIRESTORE: Delete the doc using its ID
  ...
  String _formatDate(dynamic timestamp) {
    try {
      // ✅ FIRESTORE: Timestamps come as Firestore Timestamp objects, not strings
      DateTime date;
          ...
          SafeArea(
            ...
            child: Column(
              children: [
                ...
                // ✅ FIRESTORE: StreamBuilder listens to Firestore in real time
                // Think of it like a live TV channel — whenever notes change, the list updates automatically
                Expanded(
                  child: StreamBuilder<QuerySnapshot>(
                    stream: _notesRef.orderBy('date', descending: true).snapshots(),
                    builder: (context, snapshot) {

                      // Show a spinner while loading
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xFF016274)),
                        );
                      }
                      ...
                      // ✅ FIRESTORE: docs is the list of notes from Firebase
                      ...
                      for (var doc in docs) {
                        ...
                        final noteWithId = {...note, 'id': doc.id};
                        final dateLabel = _formatDate(note['date']);

                        if (!groupedNotes.containsKey(dateLabel)) {
                          groupedNotes[dateLabel] = [];
                        }
                        groupedNotes[dateLabel]!.add(noteWithId);
                      }

                      final colors = [lightBlue, primaryYellow, lightGrey];
                      int colorIndex = 0;
                        ...
                        children: groupedNotes.entries.map((dateGroup) {
                          return Column(
                            ...
                            children: [
                              ...
                              ...dateGroup.value.map((note) {
                                final noteColor = colors[colorIndex % 3];
                                colorIndex++;
                                ...
                                return Padding(
                                  ...
                                  child: _buildNoteCard(
                                    ...
                                    color: noteColor,
                                    onTap: () => _openNoteEditor(isEdit: true, note: note),
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: Icon(Icons.edit_outlined, size: 20, color: Colors.black87),
                    ),
                    SizedBox(width: 8),
                    // ✅ NEW: Visible delete button (no more guessing long press!)
                    GestureDetector(
                      onTap: onDelete,
                      child: Container(
                        padding: EdgeInsets.all(8),
